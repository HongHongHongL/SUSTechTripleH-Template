...
int g[maxn], id[maxn], n, m, q, rt;
double c[maxn], low[maxn], vis[maxn], p[maxn], fa[maxn], ans = 0;
...
void getans() {
  ...
  while(1) {
    ...
    for(int i = 1; i <= m; i++)
      if(e[i].x != e[i].y && e[i].w < low[e[i].y])
        low[e[i].y] = e[i].w, fa[e[i].y] = e[i].x;
    low[rt] = 0; cnt = 0;
    for(int i = 1; i <= n; i++) {
      ans += low[i];
...
int ans, n, m, s, t, u, cur[maxn], d[maxn], v[maxm], c[maxm];
int pre[maxm], num[maxn], head[maxn], last[maxm], nf, cnp;
queue <int> q;
inline int read()
{
  int x=0;
...
int main()
{
  ...
  while(scanf("%lld%lld",&n,&k)!=EOF)
  {
    ...
    for(int i=1;;i++){
      ll y=i*i*n+1, yy=sqrt(y);
      if(yy*yy==i*i*n+1){keepy=i;keepx=yy;break;}
    }
    ans.a[0][0]=keepx%mod;
...
int n, T; char s[5]; db d, turn, ans; point dir, up, le, nxt; 
const db loop = pi * 2;
int main() {
  scanf("%d", &T);
  while(T--) {
    ...
    while(n--) {
      ...
      if(s[0] == 'U') 
        {dir = rotateP(dir, le, turn); up = rotateP(up, le, turn);}
      else if(s[0] == 'D') 
        {dir = rotateP(dir, le, -turn); up = rotateP(up, le, -turn);}
      else if(s[0] == 'L') 
        {dir = rotateP(dir, up, -turn);le = rotateP(le, up, -turn);}
...
namespace Tree {
  ...
  int head[maxn], fa[maxn], dep[maxn], siz[maxn], son[maxn];
  int dfn[maxn], top[maxn], cnt, idx;
...
namespace Seg {
  ...
  void pushdown(int t) {
    ...
    tr[lson] += add[t] * siz[lson];
    tr[rson] += add[t] * siz[rson];
  ...
  void modify(int l, int r, int ql, int qr, int t, int opt, int val) {
    ...
    else modify(l, mid, ql, mid, lson, opt, val), 
      modify(mid + 1, r, mid + 1, qr, rson, opt, val);
    pushup(t);
  }
  ...
  int query(int l, int r, int ql, int qr, int t) {
    ...
    return query(l, mid, ql, mid, lson) 
    + query(mid + 1, r, mid + 1, qr, rson);
  }
...
namespace kd {
  ...
  int build(int l, int r, int dir) {
    ...
    for(int i = 0; i < sigma; i++) 
      tr[mid].num[i] = tr[mid].val[i][0] = tr[mid].val[i][1] = a[mid].num[i];
    if(l < mid) {tr[mid].ch[0] = build(l, mid - 1, (dir + 1) % sigma); 
      pushup(mid, tr[mid].ch[0]);}
    if(mid < r) {tr[mid].ch[1] = build(mid + 1, r, (dir + 1) % sigma); 
      pushup(mid, tr[mid].ch[1]);}
    return mid;
  }
...
```c++
int n, m, num, a[N], w[N << 1], wl, rt[N << 8],
 ls[N << 8], rs[N << 8], sum[N << 8], st[100], ed[100], stn, edn;
char s[5];
struct node {
  int l, r, k, type;
}op[N];
...
struct Palindromic_Tree {
  ...
  int len[MAXN];
  //len[i]表示节点i表示的回文串的长度（一个节点表示一个回文串）
  int S[MAXN];//存放添加的字符
